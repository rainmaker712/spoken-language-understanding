...
class TransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, d_ff, n_heads, dropout):
        ...
        self.layers: List = nn.ModuleList(
            [
                TransformerEncoderLayer(d_model=d_model, d_ff=d_ff, n_heads=n_heads, dropout=dropout)
    ...
    def forward(self, x, sources_mask=None, sources_key_padding_mask=None):
        ...
        for layer_index, layer in enumerate(self.layers):
            x = layer(x, sources_mask=sources_mask, sources_key_padding_mask=sources_key_padding_mask,)
        x = self.layer_norm(x)
        return x
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, d_ff, n_heads, dropout):
        ...
        self.feed_forward = PositionwiseFeedForwardNetwork(d_model=d_model, d_ff=d_ff, dropout=dropout)
    ...
    def forward(self, x, sources_mask=None, sources_key_padding_mask=None):
        x, _ = self.self_attention_sublayer(
            ...
            lambda x: self.self_attention(
                query=x, key=x, value=x, attention_mask=sources_mask, key_padding_mask=sources_key_padding_mask
            ),
        )
        ...
        for layer_index, layer_cache in self.layer_caches.items():
            if layer_cache["self-attention"]:
                sample_state.layer_caches[layer_index]["self-attention"] = {
                    key: value[item : item + 1] for key, value in layer_cache["self-attention"].items()
                }
            if layer_cache["memory-attention"]:
                sample_state.layer_caches[layer_index]["memory-attention"] = {
                    key: value[item : item + 1] for key, value in layer_cache["memory-attention"].items()
                }
        return sample_state
        ...
        for layer_index in states[0].layer_caches.keys():
            if states[0].layer_caches[layer_index]["self-attention"]:
                merged_state.layer_caches[layer_index]["self-attention"] = {
                    key: torch.cat([state.layer_caches[layer_index]["self-attention"][key] for state in states], dim=0)
            ...
            if states[0].layer_caches[layer_index]["memory-attention"]:
                merged_state.layer_caches[layer_index]["memory-attention"] = {
                    key: torch.cat(
                        [state.layer_caches[layer_index]["memory-attention"][key] for state in states], dim=0
                    )
        ...
        self.layers: List = nn.ModuleList(
            [
                TransformerDecoderLayer(
                    d_model=d_model, d_ff=d_ff, n_heads=n_heads, dropout=dropout, use_memory=use_memory
                )
                for _ in range(num_layers)
            ]
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        self, query, key, value, attention_mask=None, key_padding_mask=None, cache=None,
    ):
        """
        ...
        query_heads = query_projected.view(batch_size, query_len, self.n_heads, d_head).transpose(1, 2)
        ...
        value_heads = value_projected.view(batch_size, value_len, self.n_heads, d_head).transpose(1, 2)
        ...
        if key_padding_mask is not None:
            key_padding_mask_expanded = key_padding_mask.unsqueeze(dim=1).unsqueeze(dim=1).expand_as(attention_scores)
            attention_scores = attention_scores.masked_fill(key_padding_mask_expanded, float("-inf"))
        if attention_mask is not None:
            attention_mask_expanded = attention_mask.unsqueeze(dim=0).unsqueeze(dim=0).expand_as(attention_scores)
        ...
        context_heads = torch.matmul(attention_dropped, value_heads)  # (batch_size, n_heads, query_len, d_head)
        context_sequence = context_heads.transpose(1, 2).contiguous()  # (batch_size, query_len, n_heads, d_head)
    ...
    def scaled_dot_product(self, query_heads, key_heads):
        ...
        dot_product = torch.matmul(query_heads, key_heads_transposed)  # (batch_size, n_heads, query_len, key_len)
...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def build_positional_encoding(self, length):
        ...
        div_term = torch.exp(
            torch.arange(0, self.encoding_size, 2, dtype=torch.float32) * -(math.log(10000.0) / self.encoding_size)
        )
...
def mask_from_subsequent_positions(size):
    ...
    """
    subsequent_mask = np.triu(np.full(shape=(size, size), fill_value=float("-inf"), dtype=np.float32), k=1)
